...
const double E_min = 0.0; // min. pole eletrostatyczne
const double E_step = 0.02; // krok pola
const unsigned int x_size = 30;
...
void save_to_file (double el_field, double refractive_index){
    m.lock();

    file_refractive << el_field << "  " << refractive_index << endl;
...
void set_point(double E_begin, double E_end){
    ...
    while (E_begin < E_end){ //petla po energiach
        ...
        for (unsigned int step = 0; step < MCS; step++){
            for (unsigned int i = 0; i < x_size; i++){
                for (unsigned int j = 1; j < (y_size-1); j++){
                    ...
                    if ((static_cast<double>(rand())/RAND_MAX) < min(exp(-dU), static_cast<double>(1))){
        ...
        save_to_file(E_begin,avarage(refractive_index));
...
int main (){
    ...
    double points_start = E_min;
    double points_stop = E_min + (E_max - E_min)/NO_threads;

    for (int i=0; i < NO_threads; i++){ //tworze watki
        core[i] = thread(set_point, points_start, points_stop);
        cout << i << " " << points_start << " " << points_stop << endl;
        points_start = points_stop + E_step;
        points_stop += (E_max - E_min)/NO_threads;
    }
